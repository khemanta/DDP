...
#     })  
  ...
  output$ranfor <- renderText({
    "This demonstration helps to analyze sentiment of leaders for various dates."
  })  
  output$gvis <- 
    renderGvis({
      if (input$goButton == 0)
      ...
      gvisGeoChart(pd, locationvar='loc', colorvar='SentimentType', 
                   options=list( height=400, legend = "none",
                                 ...
                                 sizeAxis ="{minSize: 5, maxSize: 5}", chartid = "Sentiment Map", backgroundColor.fill = 'grey')
                 )
